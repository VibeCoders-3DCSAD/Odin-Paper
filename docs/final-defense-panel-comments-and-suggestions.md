# Final Defense Panel Comments and Suggestions

**Project:** Development of Odin: A Personal Budget Management System Using LSTM and Isolation Forest Algorithm  
**Program:** Bachelor of Science in Computer Science Major in Application Development  
**Year/Section:** III - DCSAD  
**Presentation date:** April 21, 2026  
**Presentation time:** 2:30 PM - 3:30 PM  
**Researchers:** Stefanie S. Gabion; Joaquin Luis T. Guevarra; Alexa Joanne Paula G. San Jose; Charles Nathaniel B. Togle

## Prof. Daniel Dellosa

- Asked about user transactions.
- Asked if inputs are manual and not automatic.
  - The group confirmed that the system cannot integrate banking and e-wallet systems due to integration fees and complexity.
...
- Asked if the system is designed for regular working professionals.
  - Professor Plan corrected, saying that the system is for general users.
...
- Asked who will use the web side.
  - The group answered that the web side is for additional accessibility, for when users want to use the application on their personal computers.
...
- Asked how users are assigned to the financial behavioral profiles: Stable-Flexible, Stable-Obligated, Variable-Flexible, and Variable-Obligated.
...
- Asked why some categories are protected from reduction.
  - The group answered that categories like Essentials and Investments should be protected from reduction by the budget recommendation, as they are either essential to the user's day-to-day life or necessary to build wealth.

## Prof. Janice Pola Congzon

- Asked about Odin.
- Asked what distinguishes Odin from other personal finance and budgeting apps.
  - The group answered that other systems are "static," meaning only basic statistics are displayed.
  - The group further explained that Odin dynamically forecasts expenses, savings, and repayment goals for the next week or month, meaning the trajectory if the pattern of expenses, savings, and income is continued.
...
- Asked if the profiles are anchored to spending habits and expenses, meaning if a user were a "Flexible" type, the financial plan and budget would be designed based on that behavior.
  - The group confirmed, stating that the basis for budget building in the system is heavily defined by the profiles.
  - The group further explained that, for example, Stable-Flexible users will have larger allocations for Discretionary expenses, and Variable-Obligated users will have smaller allocations for them.
...
- Asked what it does.
  - The group explained that when users input expenses for a category that exceeds the thresholds of that category, the system will flag the transaction and alert the user.
- Asked what the next steps are after the flagging, such as whether the system will reduce allocations for that category the next time.
  - The group confirmed and noted that protected categories are excluded from the reduction.
...
> The printer/scanned copy contains handwritten signatures and dates in the signature columns. These authenticate the copy but do not change the comments and suggestions transcribed above.
