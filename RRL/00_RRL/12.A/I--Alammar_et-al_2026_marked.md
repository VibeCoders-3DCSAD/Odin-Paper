...
Scientific Reports |        (2026) 16:14180

| https://doi.org/10.1038/s41598-026-43440-9
...
m, where Aij represents the
U =
allocation of resource rj to user ui. The goal is to maximize utility

 while ensuring explanations E:
...
g (x) , where g is interpretable.

Explanation E quantifies the contribution of each feature xi to the prediction:
...
wi = ∆y, where wi is the importance of feature xi
...
where each fi is a human-understandable operation. Predictions are independent of protected attributes s :
...
where βi represents causal effects. The model provides uncertainty estimates for predictions:

y = µ

±

σ, where µ is the mean and σ the standard deviation
...
Generating minimal changes δ to alter the prediction, δsuchthatf (x + δ) = y
...
x (t) = [di (t) , pi (t) , cij (t) , ℓ (t) , τ (t) , κi (t) , hi (t)] ,

(10)

where di (t) denotes the demand of user i, pi (t) is the user’s priority or mission urgency index, cij (t) represents
the  channel  quality  between  user  i  and  resource  j,  ℓ (t)  is  the  global  network  load,  τ (t)  captures  temporal
congestion  signatures  ,  κi (t)  indicates  fairness-group  identifiers,  and  hi (t)  encodes  historical  allocation  or
dissatisfaction levels. This explicit definition clarifies the features provided as input to the predictive model and
ensures full reproducibility of the experimental setup. The learning model fθ (
) does not directly output a final
·
...
sij (t)  represents  a  differentiable  approximation  of  the  marginal  utility  gain  achieved  by
where  each  score
...
S (t),  SLIRA  computes  the  allocation  matrix  A (t)  using  an  explanation-
Given  the  predicted  score  matrix
...
-th user (emergency responder or unit)

Set of available communication resources

j-th resource (bandwidth block / slice)

Allocation of resource rj  to user ui

Allocation matrix at time step t

Capacity of resource rj

Demand of user ui at time t

Priority level of user ui

AI-based predictive allocation model

Interpretable surrogate model

Utility function of the allocation

SHAP value for user–resource pair (i, j)

LIME local feature importance

Hybrid SHAP–LIME explanation coefficient

Trade-off parameter between utility and explainability

Weight balancing global (SHAP) and local (LIME) explanations

Fairness tolerance threshold

Loss function (utility, fairness, and explanation fidelity)

Fairness-related loss component

Model parameters

Probability distribution

Mean and standard deviation (uncertainty estimates)

Number of SHAP Monte Carlo samples
...
where f  is the AI model, g is an interpretable surrogate, and
D
...
∑
where di is the demand of user ui ,pi is their priority, and Cj is the capacity of resource rj.
...
where S is a coalition of users, and ϕij explains the impact of allocating rj to ui.
...
(cid:31)
(cid:31)
where G1, G2 are protected groups, and δ is a fairness tolerance.
(cid:31)
...
(cid:31)
(cid:31)
Generate minimal changes ∆A to alter allocations for a user ui:
...
where Et is the explanation at time t.
...
current network state x (t) to an urgency-weighted score matrix fθ (x (t)); (ii) global and local explainability
...
where: Aij: Resource j allocated to user i, wij: Weight represents the priority of user i for resource j.  Under
Constraints:
...
For each user-resource pair (i, j), compute the SHAP value ϕij to quantify the contribution of user i’s demand
...
(S, A): Utility when only users in S receive resources. Interpretation:
where: S: Subset of users excluding i.
ϕij > 0 implies user i’s presence increases the system utility for resource j. Ensure allocations align with SHAP-
based fairness. For protected groups (G1, G2):
...
where  δ  is  a  fairness  tolerance  threshold.  This  penalizes  biased  allocations  toward  specific  groups.  Update
...
:  Loss  function  penalizing  unfairness  or  capacity  violations.    β :  Regularization
...
(cid:30)(cid:29)
:  Impact  of  user  i’s  demand  on  allocation.  ϕnetwork
ij

:  Impact  of  network  congestion  or
...
ij
 ...
 is an expert’s allocation and ϵ is an agreement threshold. Model uncertainty in SHAP explanations
...
i=1
∑
j) is the utility from resource(j)
...
specific  user-resource  instance  xi,  LIME  approximates  the  black-box  model  f   as  resource  allocator  with  an
...
where:
perturbations around xi. Ω (g): Regularization term enforcing simplicity.

:  Loss  function  measuring  how  well  g  approximates  f   near  xi.  πxi :  Proximity  kernel  weighting

L

LIME generates local feature importance ϕij for each user-resource pair (i, j):

where: βj: Coefficient of feature j in the interpretable model g. xij: Feature value. µj: Mean feature value in the
...
:  Loss  penalizing  unfairness  or  capacity  violations.  λ:  Regularization  parameter
where:  α:  Learning  rate.
...
where δ is a fairness threshold, generate minimal changes ∆xi to alter the allocation for user i:
...
from the range λ
[0.1,1.0]. Empirical tuning showed that λ = 0.5 provides an effective compromise between
performance and interpretability, preventing either objective from dominating the optimization process. The
SHAP–LIME  fusion  weight  γ
[0,1]  controls  the  relative  contribution  of  global  (SHAP)  and  local  (LIME)
...
: SHAP values from cooperative game theory. ϕSLIRA
...
(cid:28)
: SLIRA’s explanation coefficients. λ1, λ2:
...
Modify LIME’s proximity kernel πxi  to prioritize perturbations aligned with SHAP’s global importance:
...
where  D (xi, z)  is  the  Euclidean  distance  and  ϕSHAP
allocations using a SHAP-LIME hybrid gradient:

j

  scales  weights  by  global  feature  importance.  Update
...
[0,1]: Balances global vs. local explanations.

f i: Fairness loss.

Enforce fairness across protected groups G1, G2 using combined SHAP-LIME contributions:
...
Model uncertainty via Bayesian fusion of SHAP and LIME:
...
Generate minimal changes ∆xi to alter allocations, guided by both SHAP and LIME:
...
The  definition  of  x (t),  fθ (x (t)),  and  the  update  rule  leading  to  A (t)  establishes  the  complete  SLIRA
...
Direct Solver (Static Optimal) Convex optimization
...
SHAP–LIME + Fairness + Bayesian 0.981 ± 0.008
...
Black-Box AI AI + SHAP AI + LIME SLIRA (Full)
...
The total number of allocation decision variables is defined as N = U R. The direct solver baseline computes
allocations by solving the constrained utility maximization problem defined in Eqs. (22)–(23). Interior-point
methods for convex optimization exhibit worst-case computational complexity O(N 3) = O((U R)3), primarily
...
computational complexity becomes O(T (U R)3). This cubic dependence on the decision variable dimension
...
The predictive forward inference step has complexity O(U R). SHAP-based Monte Carlo sampling, with fixed
K = 100,  incurs  complexity  O(KF ),  while  LIME  local  surrogate  fitting,  with  fixed  P = 500,  contributes
O(P F ). Variational Bayesian uncertainty estimation scales as O(
 denotes the number of model
parameters.  Finally,  the  explanation-driven  allocation  update  and  projection  step  has  complexity  O(U R).
Because K and P  are fixed constants independent of system size, the dominant scaling term becomes O(U R).
Consequently, under dynamic operation across T  time steps, the total computational complexity of SLIRA is
O(T U R), which grows linearly with the number of allocation variables.
...
Formally,  the  asymptotic  comparison  satisfies  limN

→∞

O(N )
O(N 3) = 0,  demonstrating  that  SLIRA  grows
...
scaling comparison between solver-based convex optimization O(N 3) and the SLIRA framework O(N ). The
horizontal axis represents the number of decision variables N = U R, while the vertical axis shows normalized
computational cost on a logarithmic scale. On the log–log plot, the solver-based optimization curve exhibits
an approximate slope of 3, confirming its  cubic scaling  behavior  O(N 3), while the SLIRA curve exhibits an
...
=
 depending on the experiment. The available communication
 represent shared bandwidth blocks, with M = 5. Each resource rj is associated

U
{
resources
with a finite capacity Cj, reflecting spectrum and infrastructure limitations typical of PSNs during congestion.
...
and command units. Each user ui is characterized by:

  I.  A time-varying demand di(t), representing required data rate or communication load,
  II.  A priority level pi, reflecting mission criticality and role hierarchy,
...
n), where K is the number of sampled coalitions. This approximation preserves
...
n), where P  is
models by generating perturbed samples around each instance. Its complexity scales as
...
R). Bayesian uncertainty estimation is performed using variational
resources R, yielding a complexity of
...
set of emergency users
 depending on the experiment. Users
...
resources
  correspond  to  shared  communication  resources  such  as  bandwidth  blocks,
time–frequency resource units, or logical network slices in LTE/5G public safety systems. In the simulations,
M = 5,  and  each  resource  rj  has  a  finite  capacity  Cj,  representing  spectrum  and  infrastructure  limitations
...
(2n)  with  respect  to  the  number  of  features  n.  Such  complexity  is
exponential  worst-case  complexity
prohibitive for real-time PSN deployment. To mitigate this issue, SLIRA employs Monte Carlo sampling–based
n), where K denotes the number of sampled
SHAP approximations, reducing the computational cost to
...
n), where P  is the number of perturbations. While LIME
perturbed samples. Its complexity scales as
...
R). Bayesian uncertainty estimation is implemented using
U  and resources R, yielding a complexity of
...
under the full system constraints. The original AI baseline employs the predictive model fθ (x (t)) to generate
...
fairness and trust in decision-making. Figure 15 visualizes the impact of counterfactual changes (Δxi) on the
model’s predictions, where the y-axis represents the absolute change in output |f(xi+Δ xi)−f(xi )|. Initially, as Δxi
increases,  the  prediction  changes  decrease,  indicating  a  region  of  stability  in  the  model’s  decision  boundary.
However, beyond a certain threshold (around Δxi ≈ 1.5), the change in prediction reaches a local minimum,
...
(sLIRA)), with an overall agreement error of 0.390.
allocations (Aij
...
(human)) and the SLIRA model allocations (Aij
...
selection for 5G communication network disaster management. Sci. Program. 2022(1), 5424356.  h t t p s : / / d o i . o r g / 1 0 . 1 1 5 5 / 2 0 2 2 / 5 4
2 4 3 5 6     (2022).
...
improve employee performance through digital attendance. Riwayat Educ. J. Hist. Humanit. 7(1), 16–25.  h t t p s : / / d o i . o r g / 1 0 . 2 4 8 1 5
/ j r . v 7 i 1 . 3 6 4 1 2     (2024).
...
o r g / 1 0 . 1 0 0 7 / s 1 3 1 3 2 - 0 2 3 - 0 1 2 1 4 - y     (2024).
...
IEEE Open Journal of the Communications Society 3, 2106–2136. https://doi.org/10.1109/OJCOMS.2022.3215676 (2022).
 ...
 22.  Javed, A. R. et al. A survey of explainable artificial intelligence for smart cities. Electronics 12(4), 1020.  h t t p s : / / d o i . o r g / 1 0 . 3 3 9 0 / e l e

c t r o n i c s 1 2 0 4 1 0 2 0     (2023).
...
developing information governance standards for enhancing trust and transparency in handling customer data. SSRN  h t t p s : / / d o i .
o r g / 1 0 . 9 7 3 4 / j e r r / 2 0 2 4 / v 2 6 i 7 1 2 0 6     (2024).
...
in the Enterprise. In The International Conference on Computing, Communication, Cybersecurity & AI (pp. 420–431).  h t t p s : / / d o i .
o r g / 1 0 . 1 0 0 7 / 9 7 8 - 3 - 0 3 1 - 7 4 4 4 3 - 3 _ 2 5
...
0 . 1 0 3 8 / s 4 1 5 9 8 - 0 2 6 - 4 3 4 4 0 - 9     .
