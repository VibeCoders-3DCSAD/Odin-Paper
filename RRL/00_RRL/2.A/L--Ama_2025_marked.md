...
Preprints.org (www.preprints.org)  |  NOT PEER-REVIEWED  |  Posted: 6 August 2025
...
Lower inequality (per capita adjusted)
Less inequality than income
...
Where:  𝑅1  is the sum of the ranks for group 1 (Rural) and  𝑅2  is the sum of the ranks for group 2
(Urban). Rank the number of observations from smallest to largest. The smallest value gets a rank =
1. The largest value receives a rank of  𝑛1 +   𝑛2.    If values are tied, assign the average of the ranks for
those tied values.  𝑈1  and  𝑈2  represent how many times values in one group precede the other in
the ranking. The Mann–Whitney U test statistic is the smaller of the two:

𝑈 = min( 𝑈1 , 𝑈2)

When  both  𝑛1   and  𝑛2   are  large,  and  a  normal  distribution  is  used  to  approximate  the  U
...
Where;  𝑥 ⃗⃗⃗⃗(𝑥1, 𝑥2, … , 𝑥𝑛)   is  a  vector  of  input  (independent)  variables  or  simply  the  predictors.
𝜙𝑞,𝑝(𝑥𝑝)  are  univariate  transformations  of  input  variables  (can  be  nonlinear).  𝛷𝑞(. )  are  outer
...
𝑝=1
Where;  𝜙𝑝(𝑥𝑝)  are  smooth,  univariate  functions  (estimated  from  the  data).  𝛷(. )  is  a  smooth,
...
Where;  𝑔(. )  is  a link  function,  such  as  the  logit  function  in  logistic  regression.  𝑓1(𝑥1)  are  smooth
functions estimated for each predictor  𝑥𝑖. The right-hand side is additive, hence "Additive Models".
When modeling the expected value of a response variable  𝑌, the Generalized Additive Model (GAM)
takes the form:

𝑔(𝐸(𝑌)) = 𝛽0 + 𝑓1(𝑥1) + ⋯ + 𝑓𝑚(𝑥𝑚)
Where E(Y) is the expected value (mean) of the response variable.  𝑔(. )  is the link function (logit) for
binary outcomes.  𝛽0  is the intercept term.  𝑓1, 𝑓2, … , 𝑓𝑚  are smooth (non-parametric) functions of each
...
Variable
Predicted Secure (0)
Predicted Insecure (1)

Observed Food Secure (0)
142,690
3,409

Observed Food Insecure (1)
12,888
4,281
...
𝐹𝑂𝑂𝐷𝑖 = 𝛽0 + 𝛽1 ∙ 𝑇𝐼𝑁𝐶𝑖 + 𝜀𝑖

Where;  𝐹𝑂𝑂𝐷𝑖  is the food expenditure of the household i.  𝑇𝐼𝑁𝐶𝑖  is the total income of household i.
𝛽0  is the intercept (baseline food spending when income is zero).  𝛽1  is the change in food spending
per  peso  increase  in  income,  and  𝜀𝑖  is  the  error  term  (unexplained  variation).  However,  this  raw
...
log(𝐹𝑂𝑂𝐷𝑖) = 𝛽0 + 𝛽𝑖 ∙ log(𝑇𝐼𝑁𝐶𝑖) + 𝜀𝑖

Where;  𝑙𝑜𝑔(𝐹𝑂𝑂𝐷𝑖)  This is the log of food expenditure.  log(𝑇𝐼𝑁𝐶𝑖)This is the log of total income.  𝛽0
is the intercept (baseline log food spending).  𝛽1  is the income elasticity of food (explained below)
and  𝜀𝑖  is the error term. In a log-log model, the coefficient  𝛽1  represents elasticity.
...
Implication
Food spending increases less than
income
Proportionate change
Food spending increases more than
income
...
Where;  𝑦𝑖   is  the  original  proportion  of  food  spending  outside  the  home,  𝑛   is  the  number  of
∗  is the adjusted value constrained strictly between 0 and 1, necessary for beta
observations, and  𝑦𝑖
regression.  The  beta  regression  model  assumes  the  dependent  variable  𝑦𝑖 ∈ (0,1)and  models  the
mean  𝜇𝑖  using a logit link below

𝑙𝑜𝑔𝑖𝑡(𝜇𝑖) = 𝑙𝑛 (

) = 𝛽0 + 𝛽1 ∙ log(𝐼𝑁𝐶𝑂𝑀𝐸𝑖) + 𝛽2 ∙   𝑈𝑅𝐵𝑖 + 𝛽3 ∙   𝐹𝑆𝐼𝑍𝐸𝑖

𝜇𝑖
1 − 𝜇𝑖

Where;  𝜇𝑖   is  the  predicted  average  share  of  food  spending  outside  the  home  for  household  i.
𝛽0, 𝛽1, 𝛽2, 𝛽3  are  the  regression  coefficients.  log (𝐼𝑁𝐶𝑂𝑀𝐸)  is  the  natural  log  of  household  income.
𝑈𝑅𝐵=  1  if  urban,  0  if  rural  and  𝐹𝑆𝐼𝑍𝐸  is  the  household  size.  Instead  of  modeling  𝜇𝑖  directly,  we
model the logit of  𝜇𝑖, this transformation makes sure predictions stay between o and 1.
...
Additionally, beta regression requires  𝑦𝑖 ∈ (0,1). To adjust original values that might be 0 or 1, the
...
∗
Where;  𝑦𝑖  is the original food share (could be 0 or 1).  𝑛  is the total number of observations and  𝑦𝑖
is the transformed value used  in  the  model.  The  log-likelihood function is the heart  of  Maximum
Likelihood Estimation (MLE). In beta regression, we assume that the dependent variable  𝑦𝑖  follows
a Beta distribution:

𝑦𝑖~𝐵𝑒𝑡𝑎(𝜇𝑖, 𝜙

Where;  𝑦1  is the response variable (FOOD_SHARE_OUT_BETA).  𝜇𝑖 ∈ (0,1)  is the mean of the beta
...
Where;  𝑉𝑎𝑟(𝑦𝑖)  varies  by  both  𝜇𝑖  and  𝜙;  no constant variance.  𝜇𝑖(1 − 𝜇𝑖),  product is largest  near
...
are  tightly  clustered  around  the  mean  𝜇𝑖.  So,  High  𝜇𝑖,  low  𝜙  means  high  variance  (e.g.,  lots  of
different spending patterns) and Low  𝜇𝑖, high  𝜙  means low variance (people mostly spend close to
...
Where;  𝑙𝑜𝑔𝐿𝑚𝑜𝑑𝑒𝑙  is the log-likelihood of the fitted model.  𝑙𝑜𝑔𝐿𝑛𝑢𝑙𝑙  is the log-likelihood of the null
model (intercept only). The log-likelihood function for a beta distribution is:

𝑛

ℓ(𝜇, 𝜙 | 𝑦) = ∑ 𝑙𝑜𝑔𝛤(𝜙) − 𝑙𝑜𝑔𝛤(𝜇𝑖𝜙)

− 𝑙𝑜𝑔𝛤((1 − 𝜇𝑖)𝜙) + (𝜇𝑖𝜙 − 1)𝑙𝑜𝑔𝑦𝑖 +
...
precision.  𝑙𝑜𝑔𝛤(𝜇𝑖𝜙), 𝑙𝑜𝑔𝛤((1 − 𝜇𝑖)𝜙)   are  the  shape  component  of  the  beta  distribution.  (𝜇𝑖𝜙 −
1)𝑙𝑜𝑔𝑦𝑖   are  the  Likelihood  contribution  for  the  observation’s  actual  value  and  ((1 − 𝜇𝑖)𝜙 −
1)𝑙𝑜𝑔 (1 − 𝑦𝑖)  are  the  Complementary  contribution  (1  minus  observation).  𝑦𝑖   is  the  adjusted
proportion  of  food  expenditure  outside  the  home.  𝜇𝑖   is  the  predicted  mean  from  the  regression
...
Bairagi, S., Zereyesus, Y., Baruah, S., & Mohanty, S. (2022). Structural shifts in food basket composition of
...
Briones,  R.  (2022).  Food  and  nutrient  intake  response  to  food  prices  and  government  programs:
...
in  rural  areas—FNRI.  Business  Mirror.
